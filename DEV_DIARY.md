...
## Walkie-talkie feature: voicenotes → live walkie (full creation log)
...
- Keep peer connection alive: when the sender releases the walkie button we mute local tracks (track.enabled=false) instead of closing the RTCPeerConnection so the receive path remains valid for subsequent incoming audio.
...
- `index.html` (major client changes)
  - Added signaling client code (WebSocket connect, register, send/receive messages).
  - Implemented RTCPeerConnection handling, remote audio element creation, and ontrack playback indicator.
  - Implemented `startLocalStream`, `stopLiveTalk()`, and `stopTransmit()` (mute while keeping PC alive).
  - Added negotiation guards: `_negotiating` flag, per-target throttling, safety timeout, and cooldown when server reports target-unavailable.
  - Normalized signaling IDs to strings before sending to avoid routing failures.
  - Autoplay unlock helpers: `unlockAudio()` and `ensureEnableAudioButton()` with localStorage persistence.
  - Exposed debug helpers on `window` for easier reproducible testing: `startLocalStream`, `stopTransmit`, `stopLiveTalk`, and `window.signalingClient` public API.
  - Removed the right-hand peers UI and suppressed `renderPeersUI()` calls (we still populate `window.signalingClient.peers`).

This is long but apply_patch completed and file added. Now remove the backticks and ensure markdown renders. But the file now doesn't have backticks. Good. Now respond to user.*** End Patch# Carochat — dev diary (today)
