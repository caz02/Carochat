...
// contacts_fixed.php - clean single implementation (safe replacement)
...
function render_user_row_fixed($row, $msgs = [], $is_contact = false){
    ...
    if($q !== ''){
        ...
        if(is_array($results)){
            ...
            foreach($results as $row){
                ...
                $mydata .= render_user_row_fixed($row, $msgs, $is_contact);
            }
        } else {
...
if(is_array($contacts) && count($contacts) > 0){
    foreach($contacts as $row){
        $mydata .= render_user_row_fixed($row, $msgs, true);
    }
} else {
